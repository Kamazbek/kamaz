...
mongo.connect('mongodb://localhost:27017/zm');

const ADMINS = [292966454, 794973977, 723131094];
...
const RM_admin = {
	inline_keyboard: [
		...
		[{ text: "✏️ Лимит количества зверей", callback_data: "admin_limit" }],
...
const bot = new Telegram('1193742217:AAGSX9jWnNFK9k_dhcSrmCeEo8H0DS_A9Mk', { polling: true });
			...
			if (postfix) {
				if (postfix[0] == "C") {
					...
					if (c) {
						...
						return bot.sendPhoto(message.chat.id, `c${c.tree_id}.png`, { caption: `✅ Вы успешно активировали чек и получили зверя - <b>${(trees.find((a) => a.id == c.tree_id)).name}</b>`, parse_mode: "HTML", })
					}
				}
			}
			return message.send(`
...
🏝 <b>Чат</b> 👉 @ZooMoneyChat
💳 <b>Выплаты</b> 👉 @ZooMoneyPay
📢 <b>Новости</b> 👉 @ZooMoneyInfo`, {
				parse_mode: "HTML",
				reply_markup: {
					keyboard: $menu,
	...
	if (state[uid] == 77772 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		await User.findOneAndUpdate({ id: 1 }, { menu: JSON.stringify(bm) })
		return message.send(`Стоимость ратуши установлена в ${bm.price} рублей!`, { reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "admin_bm" }]] } });
	}
	...
	if (state[uid] == 7771222 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		return message.send(`Лимит на зверей в ${text} единиц установлен!`, { reply_markup: RM_admin_return });
	}
	...
	if (state[uid] == 7772 && ADMINS.indexOf(message.from.id) !== -1 && text != "0") {
		...
		kb.inline_keyboard.push([{ text: "➕ Выдать зверя", callback_data: "giveTree_" + u.id }, { text: "❌ Забрать зверя", callback_data: "takeTree_" + u.id }])
...
🐯 Зверей в зоопарке: <b>${user.trees.length}</b>
...
`, {
	...
	if (message.user.menu.startsWith('amountQiwi')) {
		...
		if (message.text > message.user.outbalance) return message.send('Недостаточно средств.');
		if (message.text < 100) return message.send('Введите сумму более 100 рублей!');
...
`, {
		...
		if (message.text == '♻️ Обмен') {
			return message.send(`Здесь Вы сможете обменять <b>⚡️ энергию</b> на <b>₽ рубли</b>

1000 энергии ⚡️ = 1 рубль
Минимальная сумма обмена: 1000 ⚡️ энергии
								
⚡️ <b>Ваша энергия:</b> ${message.user.fetuses.toFixed(2)}
...
🐯 <b>Зверей в зоопарке:</b> ${message.user.trees.length}
...
🤑 <b>Выведено:</b> ${message.user.payout.toFixed(2)}₽`, {
				...
				reply_markup: {
					inline_keyboard: [
						...
						[{ text: "♻️ Реинвест", callback_data: "reinvest" }, { text: "🐯 Мой зоопарк", callback_data: "trees:totalMy" }],
					]
				}
			});
		}
...
🔗 Ваша ссылка для приглашений: https://t.me/ZooMoney_bot?start=${message.from.id}
...
🎊 Вы уже пригласили: ${await User.countDocuments({ ref: message.from.id })}
		...
		if (message.text == '👑 Острова') {
			if (!message.user.clanName)
				return message.send(`
🤷 Ты пока еще не состоишь на острове!

😎 Ты можешь либо создать его сам, либо дождаться, пока кто-то тебя пригласит!
			
Цена создания острова, который может содержать в себе до 10 человек - 65 рублей!
`, {
					parse_mode: "HTML", reply_markup: {
						inline_keyboard: [
							[{ text: "➕ Создать остров (65 рублей)", callback_data: "clan_create" }],
							[{ text: "🐯 ТОП островов", callback_data: "clan_top" }],
				...
				if (clan.creator_id == uid || clan.zam_id == uid)
					reply_markup.inline_keyboard.push([{ text: "⚙️ Управление островом", callback_data: "clan_admin" }])
				reply_markup.inline_keyboard.push([{ text: "💳 Пополнить баланс острова", callback_data: "clan_payin" }])
				reply_markup.inline_keyboard.push([{ text: "🐯 ТОП островов", callback_data: "clan_top" }])
				reply_markup.inline_keyboard.push([{ text: "❗️ Статус битвы", callback_data: "clan_status" }])
				return message.send(`
🐯 <b>Ваш остров:</b> ${clan.name}\n
<b>Участники острова:</b>
Имя | Доходность
...
<b>Всего игроков в острове:</b> ${members.length + 1} из ${clan.maxMembers}
<b>Доходность острова:</b> ${roundPlus(clan.total_earn)} ⚡️/час
<b>В казне:</b> ${roundPlus(clan.balance)} ⚡️
<b>На балансе:</b> ${roundPlus(clan.bal)}₽\n
<b>Покинуть остров:</b> /leave_clan
`, {
					parse_mode: "HTML", reply_markup
				})
			}
		...
		if (state[uid] == 1601) {
			if (message.user.buybalance < 65) return message.send(`На Вашем балансе для покупок недостаточно средств для создания острова!`, { parse_mode: "HTML" });
			if ((await Clan.findOne({ name: message.text })) != null) return message.send(`Остров с таким названием уже существует!\nВведите другое название острова:`, { parse_mode: "HTML" });
			state[uid] = undefined
			...
			return message.send('Остров создан!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		...
		if (state[uid] == 160101) {
			...
			if (message.user.buybalance < sum) return message.send(`На Вашем балансе для покупок недостаточно средств для пополнения баланса острова!`, { parse_mode: "HTML" });
			state[uid] = undefined
			await User.findOneAndUpdate({ id: uid }, { $inc: { buybalance: -sum } })
			await Clan.findOneAndUpdate({ _id: clan._id }, { $inc: { bal: sum } })
			return message.send(`Баланс острова пополнен на ${sum}₽!`, { reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		...
		if (message.text.startsWith("/invite") && !message.text.startsWith("/invitezam")) {
			...
			if (members.length > clan.maxMembers - 1) return message.send('На острове закончились места!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			...
			if (us.clanName) return message.send('Пользователь уже состоит на острове!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			await bot.sendMessage(Number(message.text.split(" ")[1]), `<a href="tg://user?id=${uid}">${message.from.first_name}</a> приглашает Вас на остров <b>${clan.name}</b>:`, { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "✅ Подтвердить", callback_data: "clanAccept_" + clan._id }, { text: "❌ Отменить", callback_data: "clanDecline" }]] } })
			return message.send(`Запрос на вступление на остров отправлен <a href="tg://user?id=${Number(message.text.split(" ")[1])}">пользователю</a>!`, { parse_mode: "html", reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		...
		if (message.text.startsWith("/kick")) {
			...
			if (clan.creator_id != uid && clan.zam_id != uid) return
			if (!us) return message.send('Пользователь не найден в боте!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			if (us.clanName != clan.name) return message.send('Пользователь не состоит в Вашем острове!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			...
			await bot.sendMessage(Number(message.text.split(" ")[1]), `❌ Вы были выгнаны из острова <b>${clan.name}</b>!`, { parse_mode: "html" })
			return message.send(`Вы выгнали <a href="tg://user?id=${Number(message.text.split(" ")[1])}">пользователя</a> из острова!`, { parse_mode: "html", reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		if (message.text.startsWith("/invitezam")) {
			...
			await bot.sendMessage(us.id, `❗️ Вы стали заместителем админа острова <b>${clan.name}</b>!`, { parse_mode: "html" })
		...
		if (message.text.startsWith("/removezam")) {
			...
			await bot.sendMessage(clan.zam_id, `❗️ Вы больше не заместитель админа острова <b>${clan.name}</b>!`, { parse_mode: "html" })
		...
		if (message.text.startsWith("/leave_clan")) {
			var clan = await Clan.findOne({ name: message.user.clanName })
			if (!message.user.clanName) return message.send('Вы еще не состоите в острове!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			if (clan.creator_id == uid && !clan.zam_id) return message.send('Вы не можете выйти из своего острова не назначив заместителя!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
				...
				await bot.sendMessage(clan.zam_id, `❗️ Вы стали администратором острова <b>${clan.name}</b> по причине выхода владельца!`, { parse_mode: "html" })
			}
			...
			await bot.sendMessage(uid, `❌ Вы вышли из острова <b>${clan.name}</b>!`, { parse_mode: "html" })
			await bot.sendMessage(clan.creator_id, `<a href="tg://user?id=${uid}">Пользователь</a> вышел из Вашего острова!`, { parse_mode: "html" })
		}
...
📥 Инвестировано всего: ${Math.round(s.ref) + 45000}₽
📤 Выплачено всего: ${Math.round(s.fc) - 132475}₽
🕐 Старт бота произведен 03.01.2020
`, {
				parse_mode: "HTML",
				reply_markup: {
					inline_keyboard: [
						[{ text: "👨‍💻 Администратор", url: "https://t.me/Irina_zn" }],
						[{ text: "💬 Чат", url: "https://t.me/ZooMoneyChat" }],
						[{ text: "♻️ Пополнения и выводы", url: "https://t.me/ZooMoneyPay" }],
						[{ text: "❓ Помощь", callback_data: "help_main" }],
						[{ text: "⚡️ Новости проекта", url: "https://t.me/ZooMoneyInfo" }],
...
bot.on('callback_query', async (query) => {
	...
	if (query.data.startsWith('trees:shop')) {
		...
		if (id <= maxId + 1) {
			...
			else var bbtn = [{ text: `🛑 Вы достигли лимита в ${limit} зверей`, callback_data: getNavigationQuery(id + 1, tree.id) }]

		}
		else
			var bbtn = [{ text: `◀️ Сперва купите предыдущего зверя`, callback_data: getNavigationQuery(id - 1, tree.id) }]

		if (!tree) return bot.answerCallbackQuery(query.id, 'Что-то пошло не так...', true);

		bot.deleteMessage(message.chat.id, message.message_id)
		bot.sendPhoto(message.chat.id, `/home/bot/paid/ZM/c${tree.id}.png`, {
...
⚡️ Энергии в час: ${tree.earn}`, parse_mode: "HTML",
		...
		return bot.sendMessage(uid, 'Здесь находятся ответы на часто задаваемые вопросы, выберите одну из тем, которая вас интересует:', {
			reply_markup: {
				inline_keyboard: [
					...
					{ text: "⚡️ Звери", callback_data: "help_bogs" }],
	...
	if (query.data.startsWith('help_bogs')) {
		bot.deleteMessage(message.chat.id, message.message_id)
		return bot.sendMessage(uid, '⚡️ <b>Звери</b>\n\nЗвери - это герои, которые будут будут приносить Вам доход в виде ⚡️ Энергии, которые можно обменять на деньги\nДля сбора Энергии, зайдите в раздел «⚡️ Зоопарк» -> Мой зоопарк,\nДля обмена Энергии на рубли зайдите в раздел «♻️ Обмен» и произведите обмен', { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "help_main" }]] } })
	}
	if (query.data.startsWith('help_po')) {
		bot.deleteMessage(message.chat.id, message.message_id)
		return bot.sendMessage(uid, '📤 <b>Вывод денег</b>\n\nМинимальный вывод средств из игры: <b>20₽</b>\nЧтобы вывести средства, зайдите в раздел «🖥 Личный кабинет» ->  Вывести\n\nДеньги можно вывести на QIWI', { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "help_main" }]] } })
	}
	...
	if (query.data.startsWith('store')) {
		...
		return bot.sendMessage(uid, 'Магазин:', {
			reply_markup: {
				inline_keyboard: [
					[{ text: "🐯 Звери", callback_data: "trees:shop0" }],
					[{ text: "🐧 Пингвины", callback_data: "wildBees" }],
	...
	if (query.data.startsWith('wildBees')) {
		...
		wb.map((b) => { reply_markup.inline_keyboard.push([{ text: `🐧 Пингвин ${b.level} уровня`, callback_data: "wb_" + b._id }]) })
		...
		return bot.sendMessage(uid, `🐧 Пингвины могут появится только благодаря ратуше\n
<b>Для пингвинов доступно 5 уровней улучшения:</b>\n
Уровень | Энергии в час | Стоимость улучшения
1 уровень - 33 ⚡️ 
2 уровень - 340 ⚡️ - 120₽
3 уровень - 618 ⚡️ - 400₽
4 уровень - 982 ⚡️ - 500₽
...
<b>Ваши пингвины:</b>`, {
	...
	if (query.data.startsWith('wbCollect')) {
		if (message.user.wb_profits == 0)
			return bot.answerCallbackQuery(query.id, '🐧 Ваши пингвины пока не принесли дохода!', true);
		else {
			bot.deleteMessage(message.chat.id, message.message_id)
	...
	if (query.data.startsWith('wbUpLevel_')) {
		...
		bot.sendPhoto(message.chat.id, "/home/bot/paid/ZM/pin.jpg", {
			caption: `
🐧 <b>Пингвин ${wb.level} уровня</b> \n
🕒 <b>В зоопарке:</b> ${Math.floor((Date.now() - wb.start_time) / (1000 * 60 * 60 * 24))} дней
⚡️ <b>Энергии в час:</b> ${wbProfits[wb.level]}
💰 <b>Принёс энергии</b>: ${ wb.bee_profit}`, parse_mode: "HTML", reply_markup
		})
		return bot.answerCallbackQuery(query.id, `🔼 Уровень пингвина повышен до ${wb.level} за ${price}₽`, true);
...
🐯 Принесла <b>${ubm.beesGet}</b> из <b>4</b> пингвинов
`
		}
...
❗️️️ Ратуша - магическое место, где рождаются пингвины
...
🐯 Ратуша за период своей жизни приносит 4 пингвина!\n\n${str}`, {
			reply_markup, parse_mode: "html"
		});
...
⚡️ Энергии в час: ${earn}`, parse_mode: "HTML",
	...
	if (query.data.startsWith('trees:buy')) {
		...
		if (tree.price > message.user.buybalance) return bot.answerCallbackQuery(query.id, '🚫 Недостаточно денег для покупки.', true);
			...
			if (treesWithEqualId >= limit)
				return bot.answerCallbackQuery(query.id, `🛑 Вы достигли лимита в ${limit} зверей данного уровня`, true);
	...
	if (query.data == 'deposit') {
		...
		var b = (await User.findOne({ id: 0 })).deposit
		if (b == 0) var bt = ""
		else var bt = `\n<b>🔥 Времен��ая акция для остальных пополнений: +${b}% при любом пополнении</b>`
...
❗️ Не забудьте комментарий <code>Z${message.chat.id}</code>

💎 <b>Способ пополнения: Payeer</b>
🌐 Отправьте любую сумму на кошелек <code>${config.payeer.account}</code>
❗️ Не забудьте комментарий <code>MF${message.chat.id}</code>

📥 <b>Для пополнения с помощью других способов обращайтесь к администратору</b> @Irina_zn\n
${bt}`, {
			parse_mode: "HTML",
...
▫️ Морта ▫️ Мейсона ▫️ Шкипера ▫️ Джулиана ▫️ Глорию ▫️ Ещё одна прокрутку ▫️ 5₽ ▫️ 10₽ ▫️ 15₽
	...
	if (query.data == 'game_casino_spin') {
		...
		if (val <= 5) {
			val--
			var prize = `🐯 Зверя ${trees.find((a) => a.id == val).name}`
			giveTree(uid, val)
		}
		else if (val == 6) {
...
▫️ <b>Ваш выигрыш:</b> ${prize}`, {
	...
	if (query.data == 'game_prize') {
		...
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `🎁 <b>Подарок - Морт</b>\n
...
▫️ @ZooMoneyPay`, {
	...
	if (query.data == 'game_prize_check') {
		var res = await bot.getChatMember("@ZooMoneyPay", message.chat.id)
		if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
		...
		await message.user.save();
		return bot.sendMessage(message.chat.id, `🎁 <b>Вы получили подарок! Морт уже в Вашем зоопарке</b>`, {
			parse_mode: "HTML",
		});
	}
...
🎰 Возможный выигрыш: ${bet * 2} ₽`, {
	...
	if (query.data == 'trees:totalMy') {
		...
		if ($trees.filter((x) => x === 8).length) text += `\n\n<b>${trees.find((x) => x.id == 8).name}</b>\n\t\t▫️ Количество: ${$trees.filter((x) => x === 8).length}\n\t\t▪️ Энергии в час: ${$trees.filter((x) => x === 8).length * trees.find((x) => x.id == 8).earn}`;

		return bot.editMessageText(`📄 Список Ваших зверей в зоопарке: ⤵️${text}\n\n════════════════════\n📊 Суммарный доход зверей в час: ${total_earn.toFixed(2)}⚡️`, {
			parse_mode: "HTML",
	...
	if (query.data.startsWith('withdraw:')) {
		...
		bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b>`, { parse_mode: "HTML" })

		if (ticket.wallet.indexOf("P") == -1) { // Платёж через QIWI
			qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: 'Выплата от @ZooMoney_bot' }, () => { });
		}
...
`, {
			parse_mode: "html", reply_markup: {
				inline_keyboard: [
					[{ text: "👨‍💻 Админ", url: "https://t.me/Irina_zn" },
					{ text: "💬 Чат", url: "https://t.me/ZooMoneyChat" }],
					[{ text: "💳 Выплаты", url: "https://t.me/ZooMoneyPay" }],
	...
	if (d == "clan_create") {
		if (message.user.buybalance < 65)
			return bot.answerCallbackQuery(query.id, `На Вашем балансе для покупок недостаточно средств для создания острова!`, true);
		state[uid] = 1601
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, 'Введите название для Вашего острова:', { reply_markup: { keyboard: Cancel, resize_keyboard: true } });
	}
	...
	if (d == "clan_payin") {
		...
		state[uid] = 160101
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `💳 <b>Ваш баланс для пополнений:</b> ${message.user.buybalance}₽\nВведите сумму для пополнения баланса Вашего острова:`, { parse_mode: "html", reply_markup: { keyboard: Cancel, resize_keyboard: true } });
	}
	if (d == "clan_admin") {
		var clan = await Clan.findOne({ name: message.user.clanName })
...
<b>Доход казны:</b> ${clan.level}%
<b>Всего игроков на острове:</b> ${members.length + 1} из ${clan.maxMembers}
<b>Доходность острова:</b> ${roundPlus(clan.total_earn)} ⚡️/час
<b>В казне:</b> ${roundPlus(clan.balance)} ⚡️
<b>На балансе:</b> ${roundPlus(clan.bal)}₽\n
<b>Команды главы:</b>
Пригласить участника на остров - <code>/invite [id]</code>
Выгнать участника с острова - <code>/kick [id]</code>
...
`, {
			parse_mode: "html", reply_markup: {
				inline_keyboard: [
					...
					[{ text: `3⃣ Расширить остров до ${clan.maxMembers + 10} мест (65 рублей)`, callback_data: "clan_expand" }],
	...
	if (d == "clan_transferMe") {
		...
		var members = await User.find({ id: { $ne: uid }, clanName: clan.name })
		if (uid != clan.creator_id && uid != clan.zam_id) return bot.answerCallbackQuery(query.id, `Ошибка!`, true);
		if (clan.balance == 0) return bot.answerCallbackQuery(query.id, `Казна острова нулевая!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		...
		return bot.sendMessage(message.chat.id, `<b>${clan.balance} ⚡️ энергии</b> из казны острова начислены Вам как <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для покупок и <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для вывода`, { parse_mode: "html" });
	}
	if (d == "clan_transferAll") {
		...
		for (const i in members) {
			try {
				...
				await bot.sendMessage(members[i].id, `<b>${roundPlus(clan.balance)} ⚡️ энергии</b> из казны острова распределены между участниками острова\nВам начислено <b>${roundPlus(dole)}₽</b> на баланс для покупок и <b>${roundPlus(dole)}₽</b> на баланс для вывода`, { parse_mode: "html" });
			}
			catch { }
		}
	}
	if (d == "clan_expand") {
		var clan = await Clan.findOne({ name: message.user.clanName })
		if (uid != clan.creator_id && uid != clan.zam_id) return bot.answerCallbackQuery(query.id, `Ошибка!`, true);
		if (clan.bal < 65) return bot.answerCallbackQuery(query.id, `На балансе острова недотаточно средств!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		await Clan.findOneAndUpdate({ name: clan.name }, { $inc: { maxMembers: 10, bal: -65 } })
		await bot.sendMessage(uid, `Вы успешно расширили максимальное количество мест на острове до ${clan.maxMembers + 10}`, { parse_mode: "html" });
	...
	if (d == "clan_upLevel") {
		...
		await bot.sendMessage(uid, `Вы успешно повысили доходность казны острова до ${clan.level + 1}%`, { parse_mode: "html" });

	}
	if (d.startsWith("clanAccept")) {
		var clan = await Clan.findOne({ _id: d.split("_")[1] })
		var members = await User.find({ clanName: clan.name })
		if (members.length > clan.maxMembers - 1) return bot.answerCallbackQuery(query.id, `На острове закончились места!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		await User.findOneAndUpdate({ id: uid }, { clanName: clan.name })
		await bot.sendMessage(uid, `✅ Вы успешно вступили вна остров <b>${clan.name}</b>\nНажмите /leave_clan, чтобы покинуть остров`, { parse_mode: "html" });
		bot.sendMessage(clan.creator_id, `➕ На ваш остров вступил <a href="tg://user?id=${uid}">пользователь</a>\nВведите <code>/kick ${uid}</code>, чтобы выгнать участника из острова`, { parse_mode: "html" });
		totalClanEarnCalc()
	}
	if (d == "clanDecline") {
		bot.sendMessage(uid, ` ❌ Вы отменили заявку на вступление в остров!`, { parse_mode: "html" });
	}
...
5⃣ ${clans[4].name} | ${clans[4].total_earn} ⚡️`, { parse_mode: "html" });
			...
			return bot.sendMessage(message.chat.id, `<b>🏆 Недостаточно островов для составления топа</b>`, { parse_mode: "html" });
	...
	if (d == "clan_status") {
		...
		if (clans.length == 5)
			return bot.sendMessage(message.chat.id, `
<b>⚡️ Битва островов</b>

🕒 Битва островов происходит 5, 15 и 25-го числа каждого месяца!
🏆 Остров-победитель получает <b>⚡️ 100k энергии</b> в свою казну
...
<b>🛡 ТОП островов текущей битвы 🛡</b>
...
`, {
			...
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Введите максимально возможное количество зверей для покупки:', { reply_markup: RM_admin_return, parse_mode: "HTML" })
			state[uid] = 7771222
		}
			...
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете зверя для выдачи:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "giveTree2_" + d.split("_")[1] + "_0" }], [{ text: trees[1].name, callback_data: "giveTree2_" + d.split("_")[1] + "_1" }], [{ text: trees[2].name, callback_data: "giveTree2_" + d.split("_")[1] + "_2" }], [{ text: trees[3].name, callback_data: "giveTree2_" + d.split("_")[1] + "_3" }], [{ text: trees[4].name, callback_data: "giveTree2_" + d.split("_")[1] + "_4" }], [{ text: trees[5].name, callback_data: "giveTree2_" + d.split("_")[1] + "_5" }], [{ text: trees[6].name, callback_data: "giveTree2_" + d.split("_")[1] + "_6" }], [{ text: trees[7].name, callback_data: "giveTree2_" + d.split("_")[1] + "_7" }], [{ text: trees[8].name, callback_data: "giveTree2_" + d.split("_")[1] + "_8" }]] }, parse_mode: "HTML" })
		}

		else if (d.split("_")[0] == "giveTree2") {
			bot.deleteMessage(message.chat.id, message.message_id);
			giveTree(Number(d.split("_")[1]), Number(d.split("_")[2]))
			bot.sendMessage(Number(d.split("_")[1]), 'Вам выдан зверь: ' + trees[Number(d.split("_")[2])].name, { rparse_mode: "HTML" })
			...
			bot.sendMessage(uid, 'Выберете зверя, которого необходимо отнять:', { reply_markup: keyboard, parse_mode: "HTML" })
		}
			...
			bot.sendMessage(uid, 'Вы успешно забрали зверя у пользователя!', { reply_markup: { inline_keyboard: [[{ text: "Назад", callback_data: "takeTree_" + id }]] }, parse_mode: "HTML" })
		}

		else if (d == "a_voucher") {
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете зверя для создания чека:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "voucher_0" }], [{ text: trees[1].name, callback_data: "voucher_1" }], [{ text: trees[2].name, callback_data: "voucher_2" }], [{ text: trees[3].name, callback_data: "voucher_3" }], [{ text: trees[4].name, callback_data: "voucher_4" }], [{ text: trees[5].name, callback_data: "voucher_5" }], [{ text: trees[6].name, callback_data: "voucher_6" }], [{ text: trees[7].name, callback_data: "voucher_7" }], [{ text: trees[8].name, callback_data: "voucher_8" }]] }, parse_mode: "HTML" })
		}
		else if (d.split("_")[0] == "voucher") {
			bot.deleteMessage(message.chat.id, message.message_id);
			var cid = generateID(8)
			await Voucher.insertMany({ id: cid, tree_id: Number(d.split("_")[1]) })
			bot.sendMessage(uid, `Чек создан:\nhttps://t.me/ZooMoney_bot?start=C${cid}`, { reply_markup: RM_admin_return })
		}
		...
		for (const txnId in body.history) {
			if (lastTxnId == null) { lastTxnId = txnId; console.log(`Last TxnId set to: ${txnId}`) }
				...
				if (body.history[txnId].comment.startsWith('ZG')) {
					...
					bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
					return
				}
				...
				if (b == 0) {
					...
					bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${sum}₽</b> и получил ${roundPlus(sum * 0.15)}₽ бонусом!`, { parse_mode: "HTML" })
					...
					bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${sum}₽</b>`, { parse_mode: "HTML" })
...
setInterval(async () => {
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			...
			if (x.comment.startsWith('ZG')) {
				let id = Number(x.comment.split("ZG")[1]);
				let user = await User.findOne({ id });
				if (!user) return;
				await user.inc('game_payin', x.sum.amount);
				await user.inc('game_balance', x.sum.amount);
			...
			let id = Number(x.comment.split("Z")[1]);
			if (!id) return;
			let user = await User.findOne({ id });
			if (!user) return;
			...
			if (b > 0) {
				...
				bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!`, { parse_mode: "HTML" })
				...
				bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
...
async function beeMotherUpdater() {
	...
	for (var i in bm) {
		...
		bot.sendMessage(b.creator_id, `⚜️ Ваша ратуша принесла 4 пингвина и исчезла`)
	}
	// Выдача диких пчёл
	...
	for (var i in bm) {
		...
		bot.sendMessage(b.creator_id, `🐧 Ратуша принесла Вам пингвина!`)
	}
}
...
<b>⚡️ ${d.getDate()}.${d.getMonth() + 1}.${d.getFullYear()} была проведена битва островов!</b>\n
🏆 Победил остров <b>${clans[0].name}</b>
💰 Он получает <b>⚡️ 100k энергии</b> в казну острова\n
2 место - <b>${clans[1].name}</b> - получает <b>⚡️ 50k энергии</b>
3 место - <b>${clans[2].name}</b> - получает <b>⚡️ 30k энергии</b>
4 место - <b>${clans[3].name}</b> - получает <b>⚡️ 15k энергии</b>
5 место - <b>${clans[4].name}</b> - получает <b>⚡️ 5k энергии</b>\n
...
async function ma(){
	...
	response.data.map(async (x) => {
		if (!x.comment) return;
		if (!x.comment.startsWith('Z')) return;
		let id = Number(x.comment.split("Z")[1]);
		if (!id) return;
		let user = await User.findOne({ id });
		if (!user) return;
		...
		if (sum >= 300) {
			...
			//bot.sendMessage("@ZooMoneyPay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * 0.15)}₽ бонусом!`, { parse_mode: "HTML" })
